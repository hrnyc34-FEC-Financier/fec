...
const RelatedItemModal = ({ currentProduct, previewItem, handleStarModalClick, relatedProductStarModal }) => {
  console.log('currentProduct:', currentProduct.features );
  console.log('previewItem:', previewItem.features );
  // className="comparingPage_content"
  ...
  var contents = totalFeature.map((item, i )=> {
    ...
    return (
      <div id="comparingPage_content" key={i}>
        <a>{ aChecker }</a>
        <a>{ item }</a>
        <a>{ bChecker }</a></div>
    );
  ...
  return (
    <div id="comparingPage">
      COMPARING
      <h1>{currentProduct.name} &nbsp;&nbsp;&nbsp;&nbsp; vs &nbsp;&nbsp;&nbsp;&nbsp; {previewItem.name}</h1>
      {/* &#10003;  list 1  &#10003;<br /> */}
