    ...
    return searchEngine
      ...
      .then((res) => {
        ...
        for (let i = 0; i < relatedItemsList.length; i++) {
          if (relatedItemsList[i] === productId) relatedItemsList.splice(i, 1);
          if (relatedItemsList[i] === relatedItemsList[i + 1]) relatedItemsList.splice(i, 1);
        }

        dispatch(relatedItemList(relatedItemsList));
        if (updatedNeedArr) dispatch(addTimeSaverList(wholeData, relatedItemsList));
      })
